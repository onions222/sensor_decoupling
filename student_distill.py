...
class CoM_from_Patch_V12(nn.Module):
    """(V16 Pred 管线) (3x5 色块, 10-col 峰值, 奇偶标记) -> 18-col 全局坐标"""
...
class CoM_from_18col_Patch_V15(nn.Module):
    """(V16 GT 管线) (3x5 色块, 18-col 峰值) -> 18-col 全局坐标"""
    def __init__(self, patch_h=3, patch_w=5):
...
class SensorDataset_V16_Final(Dataset):
            ...
            try:
                ...
                for point_id, pair_data in aligned_data.items():
                    ...
                    samples_in_memory.append({
                        "raw_patch": merging_patch, "clean_patch": clean_patch, "is_odd": is_odd,
    ...
    def __getitem__(self, idx):
        return (
            self.raw_patches_gpu[idx], self.clean_patches_gpu[idx], self.is_odd_gpu[idx],
            self.peak_coords_18_gt_gpu[idx][0], self.peak_coords_18_gt_gpu[idx][1],
            self.peak_coords_10_m_gpu[idx][0], self.peak_coords_10_m_gpu[idx][1]
        )
...
if 'train_loader' not in locals() or len(train_loader.dataset) == 0:
    ...
    optimizer = optim.Adam(model_mcu.parameters(), lr=0.001)
...
JSON_FILE_PATH_FOR_VIZ = '/content/gdrive/MyDrive/decoupling-main/data/aligned_g26.json'
VIZ_PATCH_SIZE = (3, 5)
